...
def save_cover(solo_track_id: str, artwork_url: str, solo_cover_path: str) -> str:
    ...
    try:
        ...
        supabase.storage.from_("covers").upload(
            path=solo_cover_path, file=response.content
        )
    ...
    try:
        ...
        for track in playlist.tracks:
            ...
            def single_track():
                ...
                solo_track_id = disk_to_db.save_track_to_db(
                    track.title, solo_download_url, playlist_id
                )

                solo_cover_path = f"{playlist.title}/{track.artist}_{track.title}.jpg"
                solo_cover_path = sanitizer.sanitize_path(solo_cover_path)
                time.sleep(1)
                solo_cover_id = save_cover(
                    solo_track_id, track.artwork_url, solo_cover_path
                )
